...
$zoneid = $getzoneid['id'];
$table="tbl_order";
  $field_values=array("dzone","subtotal","uid","odate","p_method_id","pick_address","pick_lat","cou_id","cou_amt","o_total","pick_lng","trans_id","wall_amt","vehicleid","cat_id");
  $data_values=array("$zoneid","$subtotal","$uid","$timestamp","$p_method_id","$pick_address","$pick_lat","$cou_id","$cou_amt","$o_total","$pick_lng","$transaction_id","$wall_amt","$vehicleid","$cat_id");
...
$drop_address = mysqli_real_escape_string($lundry,$ParcelData[$i]['drop_address']);
$drop_lat = $ParcelData[$i]['drop_lat'];
$drop_lng = $ParcelData[$i]['drop_lng'];
$drop_name = $ParcelData[$i]['drop_name'];
$drop_mobile = $ParcelData[$i]['drop_mobile'];

$table="tbl_drop_points";
  $field_values=array("order_id","uid","drop_address","drop_lat","drop_lng","drop_name","drop_mobile","status");
  $data_values=array("$oid","$uid","$drop_address","$drop_lat","$drop_lng","$drop_name","$drop_mobile",'pending');
...
'contents' => $content,
'headings' => $heading,
'big_picture' => siteURL().'/multidemand/order_process_img/received.png'
);
